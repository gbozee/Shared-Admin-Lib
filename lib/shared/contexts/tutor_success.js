...
function getWorkingDataRecords(value, {
  getAdapter,
  updateState,
  state
}) {
...
function fetchTutorDetail(props, {
  ...
  return Promise.all([getWorkingDataRecords(null, {
    getAdapter,
    state,
    updateState
  }), getAdapter().fetchTutorDetail(props)]).then(data => {
...
const getUnverifiedTutors = (params, {
  getAdapter,
  state,
  updateState
}) => {
  return Promise.all([getWorkingDataRecords(null, {
    ...
    state,
    updateState
  }), getAdapter().getAllUnverifiedTutors(params)]).then(data => {
...
function approveTutor({
  ...
  return getAdapter().approveTutor(email, true, verified).then(data => {
    updateAnalytics(analytics.TUTOR_APPROVAL, agent);
    updateState({
      pending_verifications: wk
    });
    return data;
  });
}

function denyTutor(email, {
  getAdapter,
  state
}) {
  let {
  ...
  return getAdapter().approveTutor(email, false).then(data => {
    updateAnalytics(analytics.DENY_TUTOR, agent);
    return data;
  });
}

function addToWorkingDirectory({
  email,
  full_name
}, state, type) {
  let {
  ...
  return emails_only.includes(email) ? pending_verifications.map(x => {
    if (x.email === email) {
      return { ...x,
...
function notifyTutorAboutEmail({
  ...
  let data = addToWorkingDirectory({
    email,
    full_name
  }, state, workingActions.EMAIL_VERIFICATION);
  return getAdapter().notifyTutorAboutEmail(email).then(() => {
    updateState({
      pending_verifications: data
    });
...
function approveProfilePic(email, {
  state,
  updateState
}) {
  let data = removeFromWorkingDirectory(email, state, workingActions.PROFILE_VERIFICATION);
  updateState({
    pending_verifications: data
  });
...
function uploadProfilePic({
  ...
  let data = addToWorkingDirectory({
    email,
    full_name
  }, state, workingActions.PROFILE_VERIFICATION);
  return getAdapter().uploadProfilePic(email).then(() => {
    updateState({
      pending_verifications: data
    });
...
function rejectProfilePic({
  ...
  return getAdapter().rejectProfilePic(email).then(() => {
    updateState({
      pending_verifications: data
    });
...
function approveIdentification(email, {
  ...
  let data = removeFromWorkingDirectory(email, state, workingActions.ID_VERIFICATION);
  return getAdapter().approveIdentification(email).then(() => {
    updateState({
      pending_verifications: data
    });
...
function removeFromWorkingDirectory(email, state, type) {
  ...
  return emails_only.includes(email) ? pending_verifications.map(x => {
    ...
    return x;
  }).filter(x => x.actions.length > 0) : pending_verifications;
}
...
function uploadVerificationId({
  ...
  let data = addToWorkingDirectory({
    email,
    full_name
  }, state, workingActions.ID_VERIFICATION);
  return getAdapter().uploadVerificationId(email).then(() => {
    updateState({
      pending_verifications: data
    });
...
function rejectIdentification({
  ...
  return getAdapter().rejectIdentification(email).then(() => {
    updateState({
      pending_verifications: data
    });
...
function approveTutorEmail(email, {
  ...
  let data = removeFromWorkingDirectory(email, state, workingActions.EMAIL_VERIFICATION);
  return getAdapter().approveTutorEmail(email).then(() => {
    updateState({
      pending_verifications: data
    });
...
function firebaseAction(key, args) {
  return FireBase2[key](...args); // return import("../adapters/backupFirebase.js").then(module => {
