...
const fetchWithdrawals = (refresh, {
  state,
  getAdapter,
  updateState
}) => {
  ...
  return getAdapter().getAllWithdrawals().then(data => {
    updateState({
      withdrawals: data
    });
    return data;
...
const getWithdrawalDetail = (order, {
  state
}) => {
...
const makePayment = (order, {
  ...
  return getAdapter().makePayment(order).then(() => {
    updateState({
...
const deleteWithdrawal = (order, {
  ...
  return getAdapter().deleteWithdrawal(order).then(data => {
    updateState({
      withdrawals: withdrawals.filter(x => x.order !== order)
    });
  });
...
const fetchBookingTransaction = (booking_order, {
  getAdapter
}) => {
  return getAdapter().getBookingTransaction(booking_order);
};

const getWithdrawalTransactions = (withdrawal_order, {
  getAdapter
}) => {
  return getAdapter().getTransactions(withdrawal_order);
};

const fetchHiredTransactions = (props, {
  getAdapter,
  updateState
}) => {
  return getAdapter().getHiredTransactions(props).then(data => {
    updateState({
...
const getTransactionDetail = (order, {
  getAdapter,
  state
}) => {
  let {
...
const getAllOrdersForVerifiedTransactions = (value, {
  state
}) => {
  let {
