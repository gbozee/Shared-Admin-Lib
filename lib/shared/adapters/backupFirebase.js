...
if (!firebase.apps.length) {
  ...
  db = firebase.firestore(); // Initialize Cloud Firestore through Firebase
...
function appFireBase(keys) {
  ...
  return {
    saveAnalytics: (agent, data) => {
      return db.collection(analytics).doc(agent).set(data);
    },
    getAnalytics: agent => {
    ...
    saveWorkingData: (agent, data) => {
      return db.collection(storage).doc(agent).set({
        record: data
      });
    },
